...
use aptos_peer_monitoring_service_types::{
    LatencyPingRequest, NetworkInformationResponse, NodeInformationResponse,
...
async fn test_get_network_information_fullnode() {
    ...
    let expected_peers = hashmap! {peer_network_id_1 => connection_metadata_1};
    verify_network_information(
        &mut mock_client,
    ...
    let latest_network_info_response = NetworkInformationResponse {
        connected_peers: expected_peers.clone(),
        distance_from_validators: peer_distance_1,
    };
    let peer_monitoring_metadata_1 = PeerMonitoringMetadata::new(None, Some(latest_network_info_response), None);
    peers_and_metadata
    ...
    let expected_peers = hashmap! {peer_network_id_1 => connection_metadata_1.clone(), peer_network_id_2 => connection_metadata_2};
    let latest_network_info_response = NetworkInformationResponse {
        connected_peers: hashmap! {peer_network_id_1 => connection_metadata_1.clone()},
        distance_from_validators: peer_distance_1,
    };
...
mock! {
    ...
    impl DbReader for DatabaseReader {
        ...
        fn is_state_pruner_enabled(&self) -> anyhow::Result<bool>;
    }
}
