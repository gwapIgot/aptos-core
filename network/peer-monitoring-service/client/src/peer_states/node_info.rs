...
use aptos_peer_monitoring_service_types::{
    NodeInformationResponse, PeerMonitoringServiceRequest, PeerMonitoringServiceResponse,
};
...
mod test {
    ...
    /// Verifies that the latest node info response has a valid distance
    fn verify_node_info_state(
        ...
        // Get the latest node info response
        let latest_node_info_response = node_info_state.get_latest_node_info_response().unwrap();

        // Verify all the properties of the response
        assert_eq!(latest_node_info_response.git_hash, git_hash);
        assert_eq!(
