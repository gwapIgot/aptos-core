...
use aptos_api_v2::{build_api_v2_routes, ApiV2Config, build_api_v2_runtime};
...
pub fn bootstrap(
    ...
    // TODO: Temporary
    build_api_v2_runtime(context.clone(), &runtime).expect("Failed to start API v2");

    let api_v1_config = ApiV1Config::new(context.clone());
    let api_v2_config = ApiV2Config::new(context);
    let routes = build_routes(api_v1_config, api_v2_config).context("Failed to build API routes")?;
...
pub fn build_routes(api_v1_config: ApiV1Config, api_v2_config: ApiV2Config) -> Result<impl IntoEndpoint> {
